...
import { toggleCompletedPage } from "../lib/db";

interface PageCardProps {
  userId: string;
  pageNumber: number;
  initialCompleted: boolean;
  onStatusChange?: (pageNumber: number, isCompleted: boolean) => void;
}

export default function PageCard({ userId, pageNumber, initialCompleted, onStatusChange }: PageCardProps) {
  ...
  const handleToggle = async (e: React.MouseEvent) => {
    ...
    try {
      await toggleCompletedPage(userId, pageNumber, newCompleted);
      setCompleted(newCompleted);
      if (onStatusChange) {
        onStatusChange(pageNumber, newCompleted);
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="text-3xl md:text-4xl font-extrabold font-mono text-center w-full py-1.5 text-[#fdf6e3] group-hover:text-[#c9a84c] transition-colors">
        {pageNumber}
      </div>
